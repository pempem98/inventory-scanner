...
echo "Đang chạy WorkflowManager.py..."
...
python3 WorkflowManager.py >> "$LOG_FILE" 2>&1

# Kiểm tra mã thoát của lệnh vừa rồi
if [ $? -eq 0 ]; then
  echo "WorkflowManager.py đã hoàn thành thành công."
else
  echo "Lỗi: WorkflowManager.py đã thất bại. Vui lòng kiểm tra file log để biết chi tiết:"
  echo "$LOG_FILE"
  exit 1
fi
